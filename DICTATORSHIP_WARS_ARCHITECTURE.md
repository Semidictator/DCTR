...
DCTR (The Dictator's Republic) is a Web3 Gamified SocialFi platform that simulates real-world nations under satirical dictatorial regimes. Players compete through on-chain trading performance, unlocking exclusive Dynamic NFT Trophies ("Nuclear Bombs") that grant strategic leaderboard advantages.
...
└────────────────────────────────────────────────────────────────────┘
                              ↓
┌────────────────────────────────────────────────────────────────────┐
│                   Web3 Adapter Layer (ethers.js/web3.js)            │
│  ┌──────────────────┬──────────────────┬──────────────────────┐   │
...
| **Indexing** | Rust (Solana Chain Hooks) | On-chain event tracking |
...
| **Event Indexing** | Geyser Plugin / Chainbeam | Real-time transaction stream |

### 1.3 Data Flow Architecture

#### **Game Loop: Trading Streak Detection**
...
└─────────────────────────────────────────────────────────────┘
                         ↓
┌─────────────────────────────────────────────────────────────┐
│ Geyser Plugin detects transaction on-chain                  │
...
CREATE TABLE trade_events (
  ...
  INDEX(user_wallet, faction_id, block_time)
);
...
CREATE TABLE streak_records (
  ...
  INDEX(faction_id, streak_length)
);
...
CREATE TABLE nft_rewards (
  ...
  mint_address PUBKEY UNIQUE NOT NULL,
  metadata_uri VARCHAR(500),
  awarded_date TIMESTAMP DEFAULT NOW(),
  owner_wallet PUBKEY,
  is_tradable BOOLEAN DEFAULT TRUE,
  INDEX(faction_id, awarded_date)
);
